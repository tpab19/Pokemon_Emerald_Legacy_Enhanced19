...
// Only species used for actual pokemon creation, all others ignored to allow randomisation of all other details.
...
//
//  Making Changes Here Changes The Options In The UI.
//  Bypassed customisation here to use native method to give the starter. extra parameters 
//
static const struct MonChoiceData sStarterChoices[9] = 
{
    [BALL_TOP_FIRST]        = {SPECIES_MUDKIP},
    [BALL_TOP_SECOND]       = {SPECIES_TREECKO},
    [BALL_MIDDLE_FIRST]     = {SPECIES_TORCHIC},

    [BALL_TOP_THIRD]        = {SPECIES_CHIKORITA},
    [BALL_TOP_FOURTH]       = {SPECIES_TOTODILE},
    [BALL_MIDDLE_THIRD]     = {SPECIES_CYNDAQUIL},

    [BALL_MIDDLE_SECOND]    = {SPECIES_BULBASAUR},
    [BALL_BOTTOM_FIRST]     = {SPECIES_SQUIRTLE},
    [BALL_BOTTOM_SECOND]    = {SPECIES_CHARMANDER},
};
...
static void BirchCase_GiveMon() // Function that calls the GiveMon function pulled from Expansion by Lunos and Ghoulslash
{
    ...
    // Bypassed customisation to use native method to give the starter.
    gSpecialVar_Result = sStarterChoices[sBirchCaseDataPtr->handPosition].species;
}
...
static void Task_BirchCaseTurnOff(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        //BirchCaseFreeResources();
        //DestroyTask(taskId);
        StartFirstBattle_NationalDexMode();
    }
}
...
// No longer called, bypassing to allow renaming option from Birch in the Lab
...
static void Task_BirchCaseConfirmSelection(u8 taskId)
{
    if(JOY_NEW(A_BUTTON))
    {
        ...
        BirchCase_GiveMon();
        //BeginNormalPaletteFade(0xFFFFFFFF, 0, 0, 16, RGB_BLACK);
        gTasks[taskId].func = Task_BirchCaseTurnOff; // Removed - Task_BirchCaseRecievedMon
        return;
    }
    if (JOY_NEW(B_BUTTON))
    {
