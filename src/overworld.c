...
// Created function to store coordinates for Player's Secret Base Entrance
...
// Created function to Set warp destination to Player's Secret Base Entrance
...
void Overworld_PlaySpecialMapMusic(void)
{
    ...
    if (music != MUS_ABNORMAL_WEATHER && music != MUS_NONE)
    {
        ...
        else if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING) && !gSaveBlock2Ptr->optionsSurfMusic)
            music = MUS_SURF;
    }
    ...
    if (FlagGet(FLAG_DONT_TRANSITION_MUSIC) != TRUE)
    {
        ...
        if (newMusic != MUS_ABNORMAL_WEATHER && newMusic != MUS_NONE)
        {
            if (currentMusic == MUS_UNDERWATER || currentMusic == MUS_SURF)
                return;
            if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING) && !gSaveBlock2Ptr->optionsSurfMusic)
                newMusic = MUS_SURF;
        }
